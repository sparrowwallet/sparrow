...
public class AppController implements Initializable {
            ...
            if(db.hasFiles()) {
                for(File file : db.getFiles()) {
                    if(isWalletFile(file)) {
                        ...
                        openTransactionFile(network.get(), file);
                    }
                }
                success = true;
            }
    ...
    public void openTransactionFromFile(ActionEvent event) {
        ...
        //TODO:TESTNET - it would be nice to integrate this with the file choose dialog
        NetworkDialog dlg = new NetworkDialog();
        Optional<Network> network = dlg.showAndWait();
        if (network.isEmpty()) {
            return;
    ...
    private void openTransactionFile(Network network, File file) {
        ...
        if(file.exists()) {
            try {
                ...
                try {
                    Tab tab = addTransactionTab(network, name, bytes);
                    TabData tabData = (TabData)tab.getUserData();
                    ...
                    Tab tab = addTransactionTab(network, name, text);
                    tabs.getSelectionModel().select(tab);
                }
            } catch(IOException e) {
    ...
    public void openTransactionFromText(ActionEvent event) {
        //TODO:TESTNET - it would be nice to have this on the text entry form
        NetworkDialog dlg = new NetworkDialog();
        Optional<Network> network = dlg.showAndWait();
        if (network.isEmpty()) {
            return;
        ...
        if(text.isPresent() && !text.get().isEmpty()) {
            try {
                Tab tab = addTransactionTab(network.get(), null, text.get().trim());
                TabData tabData = (TabData)tab.getUserData();
    ...
    public void openTransactionFromQR(ActionEvent event) {
        ...
        QRScanDialog qrScanDialog = new QRScanDialog(network.get());
        Optional<QRScanDialog.Result> optionalResult = qrScanDialog.showAndWait();
        if(optionalResult.isPresent()) {
            QRScanDialog.Result result = optionalResult.get();
            if(result.transaction != null) {
                Tab tab = addTransactionTab(network.get(), null, result.transaction);
                tabs.getSelectionModel().select(tab);
            }
            if(result.psbt != null) {
    ...
    public void openExamples(ActionEvent event) {
        try {
            ...
            //addTransactionTab(Network.BITCOIN, "signer.psbt", "");
    ...
    private Tab addTransactionTab(Network network, String name, String string) throws ParseException, PSBTParseException, TransactionParseException {
        if(Utils.isBase64(string) && !Utils.isHex(string)) {
            return addTransactionTab(network, name, Base64.getDecoder().decode(string));
        } else if(Utils.isHex(string)) {
            return addTransactionTab(network, name, Utils.hexToBytes(string));
        }
    ...
    private Tab addTransactionTab(Network network, String name, byte[] bytes) throws PSBTParseException, ParseException, TransactionParseException {
        if(PSBT.isPSBT(bytes)) {
            PSBT psbt = new PSBT(network, bytes);
            return addTransactionTab(network, name, psbt);
        }

        if(Transaction.isTransaction(bytes)) {
            try {
                Transaction transaction = new Transaction(bytes);
                return addTransactionTab(network, name, transaction);
            } catch(Exception e) {
    ...
    private Tab addTransactionTab(Network network, String name, Transaction transaction) {
        return addTransactionTab(network, name, transaction, null, null, null, null);
    }

    private Tab addTransactionTab(Network network, String name, PSBT psbt) {
        return addTransactionTab(network, name, psbt.getTransaction(), psbt, null, null, null);
    }

    private Tab addTransactionTab(Network network, BlockTransaction blockTransaction, TransactionView initialView, Integer initialIndex) {
        return addTransactionTab(network, null, blockTransaction.getTransaction(), null, blockTransaction, initialView, initialIndex);
    }
